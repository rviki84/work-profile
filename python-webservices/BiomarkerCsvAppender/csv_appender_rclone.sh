#!/bin/bash
#
# CSV appender to run locally in a machine installed with 'Rclone' program
...
RCLONE_APP="G:/utility/rclone-v1.33-win-amd64/rclone.exe"

# if source starts with s3 download with 'Rclone' program
if [[ $SOURCE_DIR == s3:* ]]; then
  echo "Downloading ${SEARCH_FILE_NAME} files from ${SOURCE_DIR} to ${LOCAL_TMP_DIR}"
  $RCLONE_APP sync --include ${SEARCH_FILE_NAME} --exclude '*' --ignore-existing ${SOURCE_DIR} ${LOCAL_TMP_DIR}/
else
...
# if source starts with s3 upload with 'Rclone' program
if [[ $SOURCE_DIR == s3:* ]]; then
  echo "Uploading ouput ${OUTPUT_FILENAME} to ${SOURCE_DIR}"
  $RCLONE_APP sync ${LOCAL_TMP_DIR}/${OUTPUT_FILENAME} ${SOURCE_DIR}
fi
